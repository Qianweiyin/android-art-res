...
abstract class ClassAA implements InterfaceA {


    static final int[] gEE = new int[ELinkProcessType.values().length];
    static final int[] gED;

    static {
        try {
            gEE[ELinkProcessType.QR_CODE.ordinal()] = 1;
        } catch (NoSuchFieldError unused) {
        }
        try {
            gEE[ELinkProcessType.BAR_CODE.ordinal()] = 2;
    ...
    private TodoResult a(Map map, LuminanceSource lVar) {
        m mVar = new m();
        mVar.i(map);
        try {
            try {
                p a2 = mVar.a(b(lVar));
                e eVar = new e(a2.getText(), a(a2.MO()));
                mVar.reset();
                return eVar;
    ...
    private Map b(ELinkProcessType processType) {
        Set set;
        Set set2;
        ...
        if (i == 1) {
            ...
            noneOf.addAll(ClassD.cbb);
            noneOf.addAll(ClassD.caZ);
            set = ClassD.cba;
        } else {
            noneOf.addAll(ClassD.caZ);
            set2 = ClassD.cba;
    ...
    @Override
    public TodoResult a(ELinkProcessType processType, Bitmap bitmap) {
        return a(b(processType), buildLuminanceSource(bitmap));
    }

    @Override
    public TodoResult a(ELinkProcessType processType, byte[] bArr, int i, int i2, C0521a aVar) {
        Map b = b(processType);
        Rect framingRectInPreview = aVar.MK().getFramingRectInPreview();
        if (framingRectInPreview == null) {
            return null;
        }
        return a(b, new PlanarYUVLuminanceSource(bArr, i, i2, framingRectInPreview.left, framingRectInPreview.top, framingRectInPreview.width(), framingRectInPreview.height(), false));
    }

    abstract BinaryBitmap b(LuminanceSource lVar);
}
